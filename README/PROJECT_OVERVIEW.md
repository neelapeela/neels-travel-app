...
6. Trip view shows:
   ...
   - **Right column** (toggle with clock): full-height panel with date navigation (fixed) and a **scrollable** timeline (`Day Timeline` + hourly rows). Choosing a stop opens a **bottom sheet** that slides up **over the map** (left column only), not over the timeline. **`useStopSheetHeight`** measures full sheet height for the **island** and **`mapBottomInsetPx`** (overlap between sheet and **`.trip-map-pane`**, plus `animationend` / `ResizeObserver` and a `min(42vh, 360px)` fallback when overlap reads 0 mid-animation) for the map. **`flyTo` + `panBy`** centers the marker in the visible band above the sheet; **`invalidateSize({ pan: false })`** avoids Leaflet’s resize pan fighting that offset. **Map** fills the left column under the sheet overlay.
...
- `trip.css` — trip workspace layout and chrome (toolbar, map column, timeline, stop sheet, island, trip modals, markers). Imported by `TripPage.jsx` (and `MapView.jsx` for map/marker rules). Shared modal primitives (`.modal-overlay`, `.form-group`, etc.) remain in `App.css`.
